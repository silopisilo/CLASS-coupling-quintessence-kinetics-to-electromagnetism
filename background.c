...
int background_functions(
  ...
  /* LSCF starts [Vdf] */
  double phi_lscf, phi_prime_lscf;
  /* LSCF ends [Vdf] */
...
/* Scalar field */
  if (pba->has_lscf == _TRUE_) {
    phi_lscf = pvecback_B[pba->index_bi_phi_lscf];
    phi_prime_lscf = pvecback_B[pba->index_bi_phi_prime_lscf];
    pvecback[pba->index_bg_phi_lscf] = phi_lscf; // value of the scalar field phi
    pvecback[pba->index_bg_phi_prime_lscf] = phi_prime_lscf; // value of the scalar field phi derivative wrt conformal time
    pvecback[pba->index_bg_V_lscf] = V_lscf(pba,phi_lscf); //V_scf(pba,phi); //write here potential as function of phi
    pvecback[pba->index_bg_dV_lscf] = dV_lscf(pba,phi_lscf); // dV_scf(pba,phi); //potential' as function of phi
    pvecback[pba->index_bg_ddV_lscf] = ddV_lscf(pba,phi_lscf); // ddV_scf(pba,phi); //potential'' as function of phi
    pvecback[pba->index_bg_rho_lscf] = (phi_prime_lscf*phi_prime_lscf/(2*a*a) + V_lscf(pba,phi_lscf))/3.; // energy of the scalar field. The field units are set automatically by setting the initial conditions
    pvecback[pba->index_bg_p_lscf] =(phi_prime_lscf*phi_prime_lscf/(2*a*a) - V_lscf(pba,phi_lscf))/3.; // pressure of the scalar field
    rho_tot += pvecback[pba->index_bg_rho_lscf];
    p_tot += pvecback[pba->index_bg_p_lscf];
    dp_dloga += 0.0; /** <-- This depends on a_prime_over_a, so we cannot add it now! */
    //divide relativistic & nonrelativistic (not very meaningful for oscillatory models)---------------------------- > commented !!!
    //rho_r += 3.*pvecback[pba->index_bg_p_scf]; //field pressure contributes radiation ---------------------------- > commented !!!
    //rho_m += pvecback[pba->index_bg_rho_scf] - 3.* pvecback[pba->index_bg_p_scf]; //the rest contributes matter
    //printf(" a= %e, Omega_scf = %f, \n ",a, pvecback[pba->index_bg_rho_scf]/rho_tot );
  }
...
/* LSCF ends [Vdf] */
  ...
  if (pba->has_lscf == _TRUE_) {
    /** The contribution of scf was not added to dp_dloga, add p_scf_prime here: */
    pvecback[pba->index_bg_p_prime_lscf] = pvecback[pba->index_bg_phi_prime_lscf]*
      (-pvecback[pba->index_bg_phi_prime_lscf]*pvecback[pba->index_bg_H]/a-2./3.*pvecback[pba->index_bg_dV_lscf]);
    pvecback[pba->index_bg_p_tot_prime] += pvecback[pba->index_bg_p_prime_lscf];
  }
  /* LSCF starts [Vdf] */
...
int background_varconst_of_z(
  ...
  switch(pba->varconst_dep){
    ...
    /* LSCF starts [VdF] */
    case varconst_zetalambda:
      *alpha = 1.-1.e-6*pba->lscf_zeta*pba->lscf_lambda*log(1.+z); //-------------> 1.e-6 because zeta(class)=1.e6*zeta
      *me = 1.;
      break;
    /* LSCF ends [VdF] */
  }
...
int background_indices(
  ...
  pba->has_dr = _FALSE_;
  pba->has_scf = _FALSE_;
  pba->has_lscf = _FALSE_; /* LSCF starts [Vdf] */
  ...
  if (pba->flag_lscf != 0.)
    pba->has_lscf = _TRUE_; /* LSCF starts [Vdf] */
  ...
  /* LSCF starts [Vdf] */
  /* - indices for scalar field */
  class_define_index(pba->index_bg_phi_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_phi_prime_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_V_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_dV_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_ddV_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_rho_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_p_lscf,pba->has_lscf,index_bg,1);
  class_define_index(pba->index_bg_p_prime_lscf,pba->has_lscf,index_bg,1);
  /* LSCF ends [Vdf] */
  ...
  /* LSCF starts [Vdf] */
  class_define_index(pba->index_bi_phi_lscf,pba->has_lscf,index_bi,1);
  class_define_index(pba->index_bi_phi_prime_lscf,pba->has_lscf,index_bi,1);
  /* LSCF ends [Vdf] */
...
int background_initial_conditions(
  ...
  */
  ...
  /* LSCF starts [Vdf] */
  if(pba->has_lscf == _TRUE_){
    lscf_lambda = pba->lscf_lambda;
    pvecback_integration[pba->index_bi_phi_lscf] = -lscf_lambda/3.*log(1.e-10*rho_rad/(0.5*pow(lscf_lambda,2)/(3.-pow(lscf_lambda,2))*(pba->Omega0_b+pba->Omega0_cdm)*pow(pba->H0,2)));
    pvecback_integration[pba->index_bi_phi_prime_lscf] = pba->phi_ini_prime_lscf;

    class_test(!isfinite(pvecback_integration[pba->index_bi_phi_lscf]) ||
               !isfinite(pvecback_integration[pba->index_bi_phi_lscf]),
               pba->error_message,
               "initial phi = %e phi_prime = %e -> check initial conditions",
               pvecback_integration[pba->index_bi_phi_lscf],
               pvecback_integration[pba->index_bi_phi_lscf]);
  }
  /* LSCF ends [Vdf] */
...
int background_output_titles(
  ...
  /* LSCF starts [Vdf] */
  class_store_columntitle(titles,"(.)rho_lscf",pba->has_lscf);
  class_store_columntitle(titles,"(.)p_lscf",pba->has_lscf);
  class_store_columntitle(titles,"(.)p_prime_lscf",pba->has_lscf);
  class_store_columntitle(titles,"phi_lscf",pba->has_lscf);
  class_store_columntitle(titles,"phi'_lscf",pba->has_lscf);
  class_store_columntitle(titles,"V_lscf",pba->has_lscf);
  class_store_columntitle(titles,"V'_lscf",pba->has_lscf);
  class_store_columntitle(titles,"V''_lscf",pba->has_lscf);
  /* LSCF ends [Vdf] */
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++) {
    ...
    /* LSCF starts [Vdf] */
    class_store_double(dataptr,pvecback[pba->index_bg_rho_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_p_prime_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_phi_prime_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_V_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dV_lscf],pba->has_lscf,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_ddV_lscf],pba->has_lscf,storeidx);
    /* LSCF ends [Vdf] */
...
int background_derivs(
  ...
  if (pba->has_lscf == _TRUE_) {
    /** - Scalar field equation: \f$ \phi'' + 2 a H \phi' + a^2 dV = 0 \f$  (note H is wrt cosmological time)
        written as \f$ d\phi/dlna = phi' / (aH) \f$ and \f$ d\phi'/dlna = -2*phi' - (a/H) dV \f$ */
    dy[pba->index_bi_phi_lscf] = y[pba->index_bi_phi_prime_lscf]/a/H;
    dy[pba->index_bi_phi_prime_lscf] = - 2*y[pba->index_bi_phi_prime_lscf] - a*dV_lscf(pba,y[pba->index_bi_phi_lscf])/H ;
  }
  /* LSCF ends [Vdf] */
...
int background_output_budget(
  ...
  if (pba->background_verbose > 1) {
    ...
    /* LSCF starts [Vdf] */
    if (pba->has_lscf == _TRUE_) {
      class_print_species("lambda scalar Field",lscf);
      budget_other+=pba->Omega0_lscf;
    }
    /* LSCF ends [Vdf] */
...
double V_A_lscf(struct background *pba,
               double phi
               ) {
  double lscf_lambda = pba->lscf_lambda;
  double lscf_beta   = 0.;
  double lscf_A      = 1.5*pow(pba->H0,2)*pba->Omega0_cdm // -----------------> [VdF]: factor 3*H_0^2 to ensure physical density
                      *(pow(lscf_lambda,2)+2.*lscf_beta*lscf_lambda)/(3.-pow(lscf_lambda,2)-lscf_beta*lscf_lambda);

  return  lscf_A*exp((lscf_beta-3./lscf_lambda)*phi);
}

double dV_A_lscf(struct background *pba,
                double phi
                ) {
  double lscf_lambda = pba->lscf_lambda;
  double lscf_beta   = 0.;

  return (lscf_beta-3./lscf_lambda)*V_A_lscf(pba,phi);
}

double ddV_A_lscf(struct background *pba,
                 double phi
                 ) {
  double lscf_lambda = pba->lscf_lambda;
  double lscf_beta   = 0.;

  return pow(lscf_beta-3./lscf_lambda,2)*V_A_lscf(pba,phi);
}

double V_B_lscf(struct background *pba,
               double phi
               ) {
  double lscf_lambda = pba->lscf_lambda;
  double lscf_beta   = 0.;
  double lscf_B      = 0.5*pow(pba->H0,2)*(6.-pow(lscf_lambda,2)) // --------------> [VdF]: factor 3*H_0^2 to ensure physical density
                      *(pba->Omega0_lscf-pba->Omega0_cdm*(pow(lscf_lambda,2)+lscf_beta*lscf_lambda)/(3.-pow(lscf_lambda,2)-lscf_beta*lscf_lambda)
                      -pba->Omega0_b*pow(lscf_lambda,2)/(3.-pow(lscf_lambda,2)));

  return  lscf_B*exp(-lscf_lambda*phi);
}

double dV_B_lscf(struct background *pba,
                double phi
                ) {
  double lscf_lambda = pba->lscf_lambda;

  return -lscf_lambda*V_B_lscf(pba,phi);
}

double ddV_B_lscf(struct background *pba,
                 double phi
                 ) {
  double lscf_lambda = pba->lscf_lambda;

  return pow(lscf_lambda,2)*V_B_lscf(pba,phi);
}

double V_C_lscf(struct background *pba,
               double phi
               ) {
  double lscf_lambda = pba->lscf_lambda;

  double lscf_C      = 1.5*pow(pba->H0,2)*pow(lscf_lambda,2)/(3.-pow(lscf_lambda,2))*pba->Omega0_b; //-----> [VdF]: factor 3*H_0^2 to ensure physical density

  return  lscf_C*exp(-3./lscf_lambda*phi);
}

double dV_C_lscf(struct background *pba,
                double phi
                ) {
  double lscf_lambda = pba->lscf_lambda;

  return -3./lscf_lambda*V_C_lscf(pba,phi);
}

double ddV_C_lscf(struct background *pba,
                 double phi
                 ) {
  double lscf_lambda = pba->lscf_lambda;

  return pow(3./lscf_lambda,2)*V_C_lscf(pba,phi);
}
...
double V_lscf(
             struct background *pba,
             double phi) {
  return  V_A_lscf(pba,phi)+V_B_lscf(pba,phi)+V_C_lscf(pba,phi);
}

double dV_lscf(
              struct background *pba,
              double phi) {
  return dV_A_lscf(pba,phi)+dV_B_lscf(pba,phi)+dV_C_lscf(pba,phi);
}

double ddV_lscf(
               struct background *pba,
               double phi) {
  return ddV_A_lscf(pba,phi)+ddV_B_lscf(pba,phi)+ddV_C_lscf(pba,phi);
}
/* LSCF ends [Vdf] */
